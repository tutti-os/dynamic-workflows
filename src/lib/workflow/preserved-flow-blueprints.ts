...
const acceptance = loop({
  ...
  steps: [
    agent({
      ...
      prompt: "Work in the configured repository. Implement the requirement, run focused checks, and leave changes uncommitted. On later rounds fix the blockers in previousIteration. Requirement: {{requirement}} Previous iteration: {{previousIteration}}",
    }),
    agent({
      id: "review",
      ...
      output: json({ schema: {
        type: "object",
        required: ["status", "blockers", "suggestions", "checks", "unverified"],
        properties: {
      ...
      prompt: "Act as an independent reviewer in a disposable review workspace. Judge repository truth against the original requirement and do not rely on the implementer narrative. Reuse only your own prior criteria and blockers when present. Return valid JSON with status, blockers, suggestions, checks, and unverified fields. Requirement: {{requirement}} Previous reviewer record: {{previousStep}}",
    }),
  ],
...
const alignment = loop({
  ...
  steps: [
    agent({
      ...
      prompt: "Implement the requirement in the configured repository. On later rounds address the explicit Human feedback from previousIteration. Run focused checks and do not commit. Requirement: {{requirement}} Previous iteration: {{previousIteration}}",
    }),
    human({
      id: "human_review",
...
const acceptance = loop({
  ...
  steps: [
    agent({
      ...
      prompt: "Repair only the blocking findings from previousIteration against the original requirement. Inspect repository truth, run focused checks, and do not commit. Requirement: {{requirement}} Human-approved record: {{alignment}} Previous acceptance round: {{previousIteration}}",
    }),
    agent({
      id: "review",
      ...
      prompt: "Review repository truth independently against the requirement in a disposable review workspace. Human approval permits review but is not acceptance evidence. Reuse only your own prior criteria and blockers when present. Return valid JSON with status, blockers, suggestions, checks, and unverified fields. Requirement: {{requirement}} Previous reviewer record: {{previousStep}}",
    }),
  ],
  until: { source: "review", finalStatus: "PASS" },
});
...
const acceptance = loop({
  ...
  steps: [
    agent({
      ...
      prompt: "Repair only the blocking whole-change findings in previousIteration. Preserve scope and do not commit. Brief: {{brief}} Per-site record: {{migrate}} Previous acceptance: {{previousIteration}}",
    }),
    agent({
      id: "review",
      ...
      output: json({ schema: {
        type: "object",
        required: ["status", "blockers", "rejectedSites", "checks", "unverified"],
        properties: {
          status: { enum: ["PASS", "FAIL"] },
          blockers: { type: "array" },
      ...
      prompt: "Review the entire repository change independently in a disposable review workspace. Use repository truth, the migration brief, structured per-site outcomes, and only your own prior reviewer record. Return valid JSON with status, blockers, rejectedSites, checks, and unverified fields. Brief: {{brief}} Per-site record: {{migrate}} Previous reviewer record: {{previousStep}}",
    }),
  ],
  until: { source: "review", finalStatus: "PASS" },
});
