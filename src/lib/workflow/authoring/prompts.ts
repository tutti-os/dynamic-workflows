...
export function buildCreateAuthoringPrompt(input: {
  ...
  return [
    ...
    "Before drafting, make the control-flow decisions explicit: loop entry and later-iteration order, Human approve/revise paths, role session continuity, independent-review boundaries, and side-effect gates.",
...
export function buildEditAuthoringPrompt(input: {
  ...
  return [
    ...
    "Re-check every affected loop entry/order, Human decision path, session key, information boundary, and side-effect gate; do not edit prompt text in isolation when the requested behavior changes control flow.",
