...
Agent. Pass the resulting path and branch as Agent inputs. Commit and push from
that path in another Effect.
...
Every control outcome must reach `completeCycle` or `cancelCycle`. Every
