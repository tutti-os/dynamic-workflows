...
A workflow script is a single JavaScript module using only the workflow primitives: `meta`, `inputs`, `phase`, `agent`, and `loop`. The parser rejects everything else.

## Hard constraints

- Plain JavaScript only. No imports, `require`, Node APIs, `fs`, network APIs, `Date`, or `Math.random`.
- Do not concatenate prompt strings with `+`; write each prompt as one template-free string literal (only `{{...}}` placeholders are substituted).
...
export const inputs = {
  ...
  mode: { type: "enum", values: ["fast", "thorough"], required: false },
};
```
...
`phase("Title")` groups the following nodes in the UI. Use a few descriptive phases.
...
- Optional per-node fields: `agent`, `model`, `cwd`, `session`, `appendPrompt`.
...
loop({
  ...
  steps: [
    agent({ id: "coder", label: "Implement", prompt: "Implement {{requirement}}. Iteration {{iteration}}. Reviewer feedback: {{reviewer}}" }),
