...
agent({
  ...
  prompt: "You write the final audit record of this release decision. State facts plainly and take no external action — no commit, push, tag, deploy, or message; this is a record, not an execution step.\n\nInstructions:\n1. State the decision: the approver chose \"{{go_no_go.action}}\".\n2. If the decision was no_go, quote the approver's reason: {{go_no_go.values.reason}}. If it was go, say no reason was required.\n3. List the outstanding blockers plainly from the summary — every blocked check and every check that failed to run — even on a go decision, so the record shows what was accepted.\n\nOutput a short, plain audit note suitable for a release log.\n\nReadiness summary:\n{{summary}}",
});
