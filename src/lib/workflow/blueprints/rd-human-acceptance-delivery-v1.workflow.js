...
`,
      appendPrompt: `
请基于原始需求和仓库当前实际状态重新验收。独立查看当前代码与 diff，核对上一轮阻断问题是否解决，同时检查是否引入新回归。以仓库当前状态为准，不要依赖会话记忆中的旧代码内容，也不要依赖 RD 的交付摘要。若原始需求、相关代码和同一个「需要对齐」阻断都没有变化，不要重复运行无关检查或扩写相同意见，简洁确认阻断仍存在并返回 FAIL。判定标准和输出格式不变。
