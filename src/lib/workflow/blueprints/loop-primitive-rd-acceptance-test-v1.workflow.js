...
先重新锚定仓库当前状态：查看 git status 与 git diff，并对将要修改的文件做针对性重读；当会话记忆与仓库实际内容冲突时，一律以仓库为准。然后独立核对下方反馈与原始需求及实际代码是否一致，再修复其中标注为「阻断」的问题；「建议」项可顺手处理但不强制。Reviewer 不能扩展或改写原始需求：若反馈需要新增产品决策或明显超出原始范围，标注「需要对齐」并说明原因，不要自行扩大实现范围。若同一个「需要对齐」阻断没有任何新信息，不要重复修改代码或重新尝试同一方案，只需简洁重申阻断状态。
...
验收方法：
1. 先从原始需求提炼逐条、可验证的验收标准；不得自行增加产品需求或扩大范围。若下方上一轮验收记录已含验收标准清单且原始需求未变，沿用该清单逐项重验，不必重新推导；仅当需求或范围理解被证明有误时才修订清单。
2. 独立查看 git status、git diff、相关代码、测试和配置，理解仓库当前实现及其方案。实际代码是唯一实现证据。
3. 按验收标准逐项验证当前行为，运行与本次需求相关的 focused checks；无法验证的项目必须明确说明原因。复验轮次优先验证上一轮「阻断」的修复及其影响面，加上针对性回归；完整检查面（全量测试/构建等重型检查）只在你准备返回 PASS 的轮次要求，首轮仍需完整建立基线。
4. 检查实现是否完整覆盖相关调用面、是否引入明显回归，以及必要测试是否缺失。
5. 若下方附有上一轮验收记录，逐条核对其中「阻断」问题是否已解决，同时检查是否引入新问题。上一轮记录只是你自己的历史判断，不构成当前验收证据；若原始需求、相关代码和同一个「需要对齐」阻断都没有变化，不要重复运行无关检查或扩写相同意见，简洁确认阻断仍存在并返回 FAIL。
