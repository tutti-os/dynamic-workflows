...
- [`src/lib/workflow/authoring/materials/skill/dsl-reference.md`](../src/lib/workflow/authoring/materials/skill/dsl-reference.md) — the full language contract: `meta`, `inputs`, `phase`, `log`, agent nodes (`output: "json"`, runtime `agent`/`model`/`permissionMode` option templates and their run-level fallback, `{{workflow.cwd}}`/`requiresCwd` coupling), sessions and `appendPrompt`, human tasks, `loop` (including `firstIteration`, dotted-source `until`, step self-references), and `map` (dynamic and static-list sources, per-item `steps` pipelines, failure records).
