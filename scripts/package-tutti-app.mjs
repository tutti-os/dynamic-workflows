...
function bootstrapScript() {
  return [
    ...
    "  exit 1",
    "fi",
    'if [ -z "${TUTTI_APP_RUNTIME_DIR:-}" ]; then',
    '  echo "TUTTI_APP_RUNTIME_DIR is required for workflow runtime files." >&2',
    "  exit 1",
    "fi",
    "",
    'package_dir="${TUTTI_APP_PACKAGE_DIR:-$(CDPATH= cd -- "$(dirname -- "$0")" && pwd)}"',
    ...
    'mkdir -p "$DYNAMIC_WORKFLOWS_DATA_DIR" "$TUTTI_APP_RUNTIME_DIR"',
    "",
    'cd "$server_dir"',
...
function packageAgents({ cliScope, environment }) {
  return [
    ...
    "- It requires `TUTTI_APP_DATA_DIR` and `TUTTI_APP_RUNTIME_DIR`; workflow cwd access is limited to real directories staged below those app-owned directories.",
    "- The package directory is treated as read-only at runtime.",
    "- Nested node cwd values resolve from the run cwd and cannot escape the app data/runtime directories through absolute paths, `..`, or symlinks.",
