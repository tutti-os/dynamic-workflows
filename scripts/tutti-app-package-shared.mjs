  ...
  return {
    ...
    commands: [
      ...
      },
      {
        ...
        description:
          "Wait until a run reaches a stop point: a terminal status (completed/failed/canceled/interrupted) or waiting on human input. The Tutti CLI owns internal continuation checks and returns the full runs-get detail only at a stop point.",
        execution: {
          mode: "wait",
        },
        inputSchema: objectSchema(
...
export function commandsMarkdown(options = {}) {
  ...
  return [
    ...
    `tutti --json ${scope} runs wait --run-id <run-id>`,
    ...
    "- `runs wait`: block until a run reaches a stop point (terminal status or waiting on human input). Internal continuation checks are hidden by the Tutti CLI.",
...
function agentJourneyGuide(scope) {
  return [
    ...
    "",
    "### 4. Wait for a stop point",
    "",
    "```bash",
    `tutti --json ${scope} runs wait --run-id <run-id>`,
    "```",
    "",
    "`runs wait` is one durable CLI operation. Omit `--timeout-ms` to wait until a stop point; the CLI performs hidden continuation checks at most once per minute while the run remains active. At a real stop point it returns `reason` plus the full `runs get` detail. If your own orchestration needs a total deadline, pass the CLI-owned `--timeout-ms <milliseconds>`; expiry returns `reason: wait_timeout`, `timedOut: true`, and `executionContinues: true` without canceling the workflow. Handle each stop reason:",
    "",
    "**Orchestrator discipline.** Between stop points there is NOTHING for you to act on — the workflow's own gates do the supervising. Trust them: do not call `runs get` for progress, do not read partial outputs, and do not broadcast status updates while `runs wait` is active. Progress snapshots are pure context noise (high volume, zero decision value, stale on arrival) and they crowd out what you will need at the real stop point; your user can watch live progress in the run UI. Act at stop points, steer with operator notes when asked, and otherwise wait. A `wait_timeout` result means only your optional CLI deadline expired; the workflow continues, so decide explicitly whether to wait again rather than treating it as a workflow failure.",
    ...
    `# 3. wait until the workflow reaches a stop point`,
    `tutti --json ${scope} runs wait --run-id run_456`,
