...
function agentJourneyGuide(scope) {
  return [
    ...
    "```",
    "",
    "`--inputs` is a JSON object string keyed by input name. `--agent` is the exact target id for nodes without their own agent (discover with `tutti --json agent list`); omit it and nodes fall back to `mock` for a safe smoke run. The response's `value.run.id` is the run id you drive from here. `--cwd` must resolve INSIDE the configured cwd root — read it from `status`'s `cwdRoot`; a path outside it (e.g. a bare `/tmp/...`) is rejected. Create any scratch working directory under `cwdRoot`.",
    "",
